...
const GLuint NumObjects = 3; // Number of objects types in the scene
...
std::vector<Vertex> Lines[NumObjects - 1];
...
void initOpenGL(void) {
	...
	createVAOs(Lines[1].data(), nullptr, sizeof(Vertex) * Lines[1].size(), 0, 1);
...
void createObjects(void) {
	...
	float subColor[] = {1.0f, 0.0f, 1.0f, 1.0f};
...
void recalculateSubdivde(void) {
	...
	NumSubVerts = IndexCount * std::exp2(SubLevel);
	if(k1.size() != NumSubVerts) {
		k1.clear();
		for(int i = 0; i < Lines[1].size(); i++) {
	...
	float subColor[] = {0.0f, 0.0f, 1.0f, 1.0f};

	Lines[1].clear();
	Lines[1].resize(NumSubVerts);

	std::cout << NumSubVerts << std::endl;

	int i_2 = 0;
	for(int i = 0; i < NumSubVerts; i += 2) {
		float arr[4];
		((k1[i_2] * 3.0f + k1[i_2+1])/4.0f).toArray(arr);
		std::cout << arr[0] << " " << arr[1] << " " << arr[2] << " " << arr[3] << std::endl;
		Lines[1][i].SetCoords(arr);
		Lines[1][i].SetColor(subColor);
		((k1[i_2] + k1[i_2+1 % k1.size()] * 3.0f)/4.0f).toArray(arr);
		std::cout << arr[0] << " " << arr[1] << " " << arr[2] << " " << arr[3] << std::endl;
		Lines[1][i+1 % NumSubVerts].SetCoords(arr);
		Lines[1][i+1 % NumSubVerts].SetColor(subColor);
